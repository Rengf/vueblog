  ...
  <div class="header">
    <div class="loginBar">
      ...
      <div class="showLogined" v-else>
        <router-link class="userName" :to="{path:'/Admin'}" v-if="isAdmin">
          <img :src="HeadPortrait" alt="HeadPortrait" class="HeadPortrait" />管理员
        </router-link>
        <router-link class="userName" :to="{path:'/HeadPortrait'}" v-else>
          <img :src="HeadPortrait" alt="HeadPortrait" class="HeadPortrait" />
          {{username}}
        </router-link>
...
export default {
  ...
  methods: {
    logout() {
      axios.get("152.136.137.112:3000/api/user/logout").then(
        response => {
